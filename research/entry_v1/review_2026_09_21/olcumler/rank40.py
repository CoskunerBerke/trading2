# -*- coding: utf-8 -*-
"""ADAY SIRALAMA DENEYI — 40 coinde slot kime gitmeli? (2026-09-19)

SORU: 40 coini 3 risk slotuyla kosturmak sonucu KOTULESTIRDI (olculdu: 40 coin lehte 2 /
aleyhte 4, ortalama R 6'nin 5'inde dusuk, M2 bilesik +216% -> +100%). Sebep evren buyuklugu
mu, yoksa adaylari SIRALAMAMAK mi?

UC KOL — hepsi 40 coin, ayni pencere/kadans/tohum/arsiv:
  A) uretim       : candidate_order=None -> tek gecis, arsiv sirasi (CANLI DAVRANIS)
                    Bu kol `run40.json` icindeki "40coin" kayitlarindan GELIR, tekrar kosulmaz.
  B) kontrol      : iki fazli yol, anahtar = -arsiv indeksi (SIRA KORUNUR)
                    "Iki fazli olmak"in tek basina etkisini ayirir. Bu kol OLMADAN, C ile A
                    arasindaki fark iki degiskeni birden tasir — bu projede "bolme deney
                    degildir" dersi tam olarak bu.
  C) siralanmis   : iki fazli yol, anahtar = (kapanis - cikis esigi) / ATR14
                    Birimsiz "sinyal kac ATR uzakta" olcusu; coinler arasi KARSILASTIRILABILIR.
                    M2 esigi 28 gun onceki kapanis, T2 esigi EMA200.

Anahtar `act` sozlugunden okunur (`signal_close`, `ref_close`/`ema200`, `atr14`) — kural
bunlari zaten karara koyuyor, IKINCI bir formul hesaplanmaz.
"""
...
OUT = Path(__file__).resolve().parent / "rank40.json"
...
def _sira_koru(_sym, _act):
    """KONTROL: her adaya ayni deger -> kararli siralama arsiv sirasini KORUR."""
    return 0.0
...
def main():
    ...
    for wname, dfrom, dto in WINDOWS:
        for rule in ("m2_tsmom28", "t2_trend_regime"):
            for kol, key in (("B_kontrol", _sira_koru), ("C_siralanmis", _guc)):
                rid = "rk_%s_%s_%s" % (rule.split("_")[0], wname.lower(), kol.split("_")[0].lower())
                if rid in DONE:
                    print("%-3s %-16s %-12s -> ATLANDI" % (wname, rule, kol), flush=True)
                    continue
                ...
                results.append(row)
                _emit()
                print("%-3s %-16s %-12s -> %s" % (wname, rule, kol,
